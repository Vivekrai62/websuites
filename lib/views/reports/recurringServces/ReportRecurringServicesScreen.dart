...
import '../../../viewModels/reports/reportRecurring/ReportRecurringServicesViewModel.dart';
...
class ReportRecurringServicesScreen extends StatefulWidget {
  final GlobalKey<ScaffoldState> scaffoldKey;

  const ReportRecurringServicesScreen({
    super.key,
    required this.scaffoldKey,
  });

  @override
  _ReportRecurringServicesScreenState createState() =>
      _ReportRecurringServicesScreenState();
}

class _ReportRecurringServicesScreenState
    extends State<ReportRecurringServicesScreen> {
  final ReportRecurringServicesViewModel _viewModel =
      Get.put(ReportRecurringServicesViewModel());
  ...
  Future<void> _refreshProductList() async {
    if (mounted) {
      ...
      _viewModel.reportRecurringServiceItems.clear();
      _viewModel.loading.value = true;

      await _viewModel.fetchReportRecurringServices(context,
          forceRefresh: true);
    }
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _viewModel.resetRequestModel();
      _viewModel.fetchReportRecurringServices(context);
    });
  }
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: Obx(() {
              debugPrint(
                  'Rendering with ${_viewModel.reportRecurringServiceItems.length} items');
              if (_viewModel.loading.value &&
                  _viewModel.reportRecurringServiceItems.isEmpty) {
                return const Center(child: CircularProgressIndicator());
              }

              final recurring = _viewModel.reportRecurringServiceItems;

              if (recurring.isEmpty && !_viewModel.loading.value) {
                          ...
                          if (isFloatingButtonClicked)
                            Padding(
                              ...
                              child: CreateNewLeadScreenCard2(
                                ...
                                onSearch: (value) {
                                  _viewModel.searchServices(value);
                                },
                          ...
                          GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Column(
                                children: [
                                  ContainerUtils(
                                    // Removed fixed height to allow content to wrap naturally
