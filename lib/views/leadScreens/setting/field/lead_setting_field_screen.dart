...
class SettingUserProfileController extends GetxController {
  ...
  Future<void> leadSettingColumnLabel() async {
    try {
      loading.value = true;
      final List<LeadSettingCustomLeadLabel> fetchedLabels = await _api.columnSettingLeadLabel();
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(field.type.toString(),style: TextStyle(  color: DarkMode.backgroundColor(context),),),
                ),
...
class _LeadSettingFieldScreenBottomSheetState extends State<LeadSettingFieldScreenBottomSheet> {
  ...
  // Add SettingUserProfileController
  final SettingUserProfileController _userProfileController = Get.put(SettingUserProfileController());
  RxList<String> fieldForLabels = <String>[].obs;

  @override
  void initState() {
    super.initState();
    _patternController.text = _getDefaultPattern(_selectedType);
    // Fetch lead column labels
    _loadFieldLabels();
  }

  Future<void> _loadFieldLabels() async {
    try {
      await _userProfileController.leadSettingColumnLabel();
      fieldForLabels.assignAll(
        _userProfileController.labels.map((label) => label.fieldLabel ?? '').toList(),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load field labels: $e')),
      );
    }
  }
  ...
  void dispose() {
    ...
    Get.delete<SettingUserProfileController>();
    super.dispose();
  }
  ...
  Widget _buildFieldDetailsScreen() {
    final textBasedTypes = ['Text', 'Email', 'Number', 'Phone', 'Percent'];
    final picklistTypes = ['Picklist (Custom)'];
    final booleanTypes = ['Checkbox', 'Radio'];
    final dateTypes = ['Date', 'DateTime', 'Time'];
    final fileTypes = ['Image'];
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            Obx(() => _userProfileController.loading.value
                ? const Center(child: CircularProgressIndicator())
                : CommonTextField(
              hintText: 'Select Field For',
              isMultiSelect: false,
              categories: fieldForLabels,
              value: _fieldForController.text.isEmpty ? null : _fieldForController.text,
              onCategoriesChanged: (List<String> selected) {
                if (selected.isNotEmpty) {
                  _fieldForController.text = selected.first;
                  setState(() {
                    _hasFieldForError = false;
                  });
                }
              },
              hasError: _hasFieldForError,
              errorMessage: _hasFieldForError ? "Field For is required" : null,
              borderColor: _hasFieldForError ? Colors.red : AllColors.mediumPurple,
              suffixIcon: _hasFieldForError
                  ? Icon(Icons.error_outline, color: AllColors.darkRed)
                  : null,
            )),
          ],
        ),
          ...
          Column(
            ...
            children: [
              ...
              CommonTextField(
                ...
                isDateField: true,
                prefixIcon: Icon(Icons.date_range_sharp, color: AllColors.mediumPurple, size: 20),
              ),
            ],
          ),
          const SizedBox(height: 12),
    ...
    return Column(
      ...
      children: [
        ...
        CommonTextField(
          ...
          maxLines: maxLines,
          borderColor: isFieldLabel || hasError ? Colors.red : AllColors.mediumPurple,
          hasError: hasError,
